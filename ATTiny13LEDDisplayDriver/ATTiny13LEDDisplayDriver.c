...
static uint8_t ctc_toggle_OC0A_cycles_counter;
...
void __config_interrupts_sleeping_mode_and_ports (void) {
	...
	// that are required to refresh the LED display.
	ctc_toggle_OC0A_cycles_counter = 37;
	...
	// 4. INT0 interrupt to be generated on a rising edge.
...
static uint16_t frequency_counter_and_speed_buffer = 0;
ISR (INT0_vect) {
	cli();
	frequency_counter_and_speed_buffer++;
	sei();
}
...
void updateDisplaySequence() {
	...
	frequency_counter_and_speed_buffer = speed;
	
	// get the digits.
	speed_digits[0] = speed_digits[1] = speed_digits[2] = 0;
	while (frequency_counter_and_speed_buffer >= 100) {
		frequency_counter_and_speed_buffer-=100;
		speed_digits[0]++;
	}
	while (frequency_counter_and_speed_buffer >= 10) {
		frequency_counter_and_speed_buffer-=10;
		speed_digits[1]++;
	}
	speed_digits[2] = frequency_counter_and_speed_buffer;
	...
	// reset frequency.
	frequency_counter_and_speed_buffer = 0;
}
...
ISR (TIM0_COMPA_vect) {
	...
	if (!(PORTB & _BV(PB0))) {
		if (++ctc_toggle_OC0A_cycles_counter == 38) {
			// a second has passed, let's initiate updating the display.
			ctc_toggle_OC0A_cycles_counter = 0;
			...
			// set the start bit.
			PORTB |= _BV(PB3);
			} else if (ctc_toggle_OC0A_cycles_counter == 37) {
			// done with updating the display.
			// disable data.
			PORTB |= _BV(PB2);
			} else {
			PORTB &= ~_BV(PB3);
			if (ctc_toggle_OC0A_cycles_counter < 25) {
